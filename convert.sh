...
$FONTFORGE_BIN -lang=ff -c 'Open($1); Generate($2)' sources/Acathist-Regular.sfd sources/Acathist-Regular.ufo
...
FONTINFO=`cat sources/Acathist-Regular.ufo/fontinfo.plist`
echo "${FONTINFO/    <key>openTypeOS2Type<\/key>/    <key>openTypeOS2Selection</key>$'\n'    <array>$'\n'      <integer>7</integer>$'\n'    </array>$'\n'    <key>openTypeOS2Type</key>}" > sources/Acathist-Regular.ufo/fontinfo.plist
